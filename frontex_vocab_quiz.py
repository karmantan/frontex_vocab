...
def get_or_create_stats(conn:sqlite3.Connection,player_id:str,item_id:int)->sqlite3.Row:
    ...
    if row:
        return row
    conn.execute("insert into player_item_stats(player_id,item_id,seen,correct,wrong,streak,last_wrong_at) values(?,?,?,?,?,?,?)",(player_id,item_id,0,0,0,0,None))
...
def next_item(deck_id:int,payload:NextRequest)->dict[str,Any]:
    with db() as conn:
        ...
        return {"item_id":chosen["id"],"category":chosen["category"],"german":chosen["german"],"stats":{"seen":stat["seen"],"correct":stat["correct"],"wrong":stat["wrong"],"streak":stat["streak"]},"weight":weight}
...
def submit_answer(deck_id:int,payload:AnswerRequest)->dict[str,Any]:
    with db() as conn:
        ...
        conn.execute("update player_item_stats set seen=?,correct=?,wrong=?,streak=?,last_wrong_at=? where player_id=? and item_id=?",(seen,correct,wrong,streak,last_wrong_at,payload.player_id,payload.item_id))
        return {"correct":is_correct,"correct_answer":row["english"],"category":row["category"],"stats":{"seen":seen,"correct":correct,"wrong":wrong,"streak":streak}}
...
def list_mistakes(deck_id:int,player_id:str,category:str|None=Query(default=None))->dict[str,Any]:
    with db() as conn:
        ...
        items=[]
        for row in rows:
            items.append({"item_id":row["id"],"category":row["category"],"german":row["german"],"english":row["english"],"seen":row["seen"],"correct":row["correct"],"wrong":row["wrong"],"streak":row["streak"],"last_wrong_at":row["last_wrong_at"],"weight":compute_weight(row,row["manual_weight"])})
        return {"items":items}
